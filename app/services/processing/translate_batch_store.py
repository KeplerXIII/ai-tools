...
_KEY_PREFIX = "processing:translate_batch:"
_TTL_SEC = 7 * 24 * 60 * 60
...
async def init_translate_batch(batch_id: str, *, scanned: int) -> None:
...
async def inc_translate_batch_counter(batch_id: str, field: str) -> None:
    redis = aioredis.from_url(settings.saq_queue_url, encoding="utf-8", decode_responses=True)
    key = _batch_key(batch_id)
    try:
...
async def get_translate_batch(batch_id: str) -> dict[str, int] | None:
    redis = aioredis.from_url(settings.saq_queue_url, encoding="utf-8", decode_responses=True)
    key = _batch_key(batch_id)
    try:
